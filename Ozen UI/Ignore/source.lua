...
function Ozen:createNotifier(title, message, duration)
    ...
    if userInputService.TouchEnabled then
        notification.Size = UDim2.new(1, -40, 0, 100)
    ...
    -- Animation
    notification.Position += UDim2.new(0, 0, 0, notification.AbsoluteSize.Y + 50)
    tweenService:Create(notification, TweenInfo.new(0.3), {
        Position = notificationOffset
    }):Play()
    
    -- Auto-remove system
    local notifications = self.notifications or {}
    table.insert(notifications, notification)
    self.notifications = notifications
    
    -- Update positions for existing notifications
    for i, notif in ipairs(notifications) do
        local targetY = notificationOffset.Y.Offset - 
                      (i-1) * (notification.AbsoluteSize.Y + notificationSpacing)
        tweenService:Create(notif, TweenInfo.new(0.3), {
            Position = UDim2.new(notificationOffset.X.Scale, notificationOffset.X.Offset,
                               notificationOffset.Y.Scale, targetY)
        }):Play()
    end
    ...
    if duration then
        delay(duration, remove)
    end
    
    -- Mobile-friendly tap to close
    if userInputService.TouchEnabled then
